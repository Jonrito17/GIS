var mapView = new ol.View({
    center: ol.proj.fromLonLat([37.237825, -3.232492]),
    zoom: 4.5
});
...
var LyamungoDsTile = new ol.layer.Tile({
    title: "Lyamungo",
...
map.addLayer(LyamungoDsTile);

var NetworkTile = new ol.layer.Tile({
    title: "Network",
    source: new ol.source.TileWMS({
...
map.addLayer(NetworkTile);

var overlayGroup = new ol.layer.Group({
    title: 'Overlays',
    fold: true,
    layers: [LyamungoDsTile, NetworkTile]
});
...
//$(html(ol.proj.toLonLat(map.getView().getCenter())[1].toFixed(6) + ", " + ol.proj.toLonLat(map.getView().getCenter())[0].toFixed(6));
//$("#spanAccuracy").html("");
...
map.on('singleclick', function (evt) {
        ...
        var url = LyamungoDsTile.getSource().getFeatureInfoUrl(evt.coordinate, resolution, 'EPSG:3857', {
            'INFO_FORMAT': 'application/json',
            'propertyName': 'Lyamungo,Network'
        });
        ...
        if (url) {
            $.getJSON(url, function (data) {
                ...
                content.innerHTML = "<h3> Lyamungo : </h3> <p>" + props.Lyamungo.toUpperCase() + "</p> <br> <h3> Network : </h3> <p>" +
                    props.Network.toUpperCase() + "</p>";
...
$(function () {
    ...
    document.getElementById('attQryRun').onclick = function () {
        ...
        console.log(layer,Attribute,text);
        ...
        //} else if (operator.options.selectedIndex <= 0) {
            ...
            var url = "http://localhost:8082/geoserver/FieldSurvey/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + value_attribute + "+" + value_operator + "+'" + value_txt + "'&outputFormat=application/json"
            // console.log(url);
...
function newaddRowHandlers() {
    var table = document.getElementByName("attQryTable");
    var rows = document.getElementByName("attQryTable").rows;
